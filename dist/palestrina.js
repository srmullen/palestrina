...
},{"./scale":8,"lodash":2}],4:[function(require,module,exports){
"use strict";
...
var _fractionJs2 = _interopRequireDefault(_fractionJs);
...
function calculateTimes(phrase) {
	return _lodash2["default"].map(_lodash2["default"].reduce(phrase, function (acc, note) {
		var previous = _lodash2["default"].last(acc);
		if (previous) {
			note.time = new _fractionJs2["default"](previous.duration).add(previous.time);
		} else {
			note.time = 0;
		}
 ...
 */
function phrase() {
	var durations = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
	var pitches = arguments.length <= 1 || arguments[1] === undefined ? [] : arguments[1];
	var velocity = arguments.length <= 2 || arguments[2] === undefined ? [] : arguments[2];

	// const ph = new Array(durations.length);
	var ph = new Array(Math.min(durations.length, pitches.length));
...
function accompany() {
	...
	return _lodash2["default"].sortBy(_lodash2["default"].flatten(phrases), function (n) {
		return n.time;
	});
}
...
function after(wait, notes) {
	var time = new _fractionJs2["default"](wait);
	return _lodash2["default"].map(notes, function (note) {
...
function having(prop, values, phrase) {
	return _lodash2["default"].map(phrase, function (event, i) {
		var property = {};
		property[prop] = values[i];
		return _lodash2["default"].assign({}, event, property);
	});
}
...
function where(prop, fn, phrase) {
	var maybeDo = maybe(fn);

	return _lodash2["default"].map(phrase, function (event) {
		var property = {};
...
function wherever(applicable, k, f, notes) {
	return _lodash2["default"].map(notes, function (note) {
		var n = _lodash2["default"].clone(note);
		if (applicable(n)) {
			n[k] = f(n[k]);
		}
		return n;
...
function but(start, end, variation, phrase) {
	var variationObject = _lodash2["default"].mapKeys(_lodash2["default"].take(variation, end - start), function (val, k) {
		return +k + start;
	});
	return calculateTimes(_lodash2["default"].assign([], phrase, variationObject));
}

/* Reapeats notes n times */
function times(n, phrase) {
	return calculateTimes(_lodash2["default"].flatten(_lodash2["default"].times(n, function () {
		return _lodash2["default"].map(phrase, _lodash2["default"].clone);
	})));
}

var mapClone = _lodash2["default"].partialRight(_lodash2["default"].map, _lodash2["default"].clone);
...
function bpm(tempo) {
	return function (beat) {
		var time = new _fractionJs2["default"](beat, tempo);
...
function duration(notes) {
	return _lodash2["default"].sum(notes, function (n) {
		return n.duration;
	});
}
...
function rhythm() {
	...
	return phrase(durations, _lodash2["default"].times(durations.length, function () {
		return null;
	}));
}
...
function tempo(timing, notes) {
	return where("time", timing, notes.map(function (note) {
		var clone = _lodash2["default"].clone(note);
...
function accelerando(from, to, by) {
	function rate(t) {
		if (from >= t) {
			...
			var _duration = to - from,
			    position = t - from,
			    completion = position / _duration,
			    extent = by - 1,
...
exports.accompany = accompany;
exports.having = having;
exports.is = is;
exports.all = all;
exports.where = where;
exports.wherever = wherever;
exports.but = but;
exports.times = times;
exports.then = then;
exports.mapthen = mapthen;
exports.after = after;
exports.bpm = bpm;
...
},{"fraction.js":1,"lodash":2}],6:[function(require,module,exports){
"use strict";
...
},{"./chord":3,"./live":4,"./melody":5,"./pitch":7,"./scale":8}],7:[function(require,module,exports){
"use strict";
...
function noteInfo(_ref) {
    var pitchClass = _ref.pitchClass;
    var octave = _ref.octave;
...
function noteToMidi(note) {
    if ((0, _lodash.isString)(note)) {
        var n = parsePitch(note);
        var interval = NOTES[n.pitchClass];
        return octaveNoteToMidi(n.octave, interval);
    } else {
        var interval = NOTES[note.pitchClass];
